...
s3_bucket = "llm-ec2-bucket-project"
s3 = boto3.client("s3")
tokenizer_path = "tokenizer/"
local_tokenizer_path = "/opt/ml/input/data/tokenizer/"
data_dir = "/opt/ml/input/data/"
model_dir = "/opt/ml/model/"
os.makedirs(model_dir, exist_ok=True)
s3.download_file(s3_bucket, tokenizer_path + "vocab.json", local_tokenizer_path + "vocab.json")
s3.download_file(s3_bucket, tokenizer_path + "merges.txt", local_tokenizer_path + "merges.txt")
s3.download_file(s3_bucket, "data/all_hin_articles.json", "/opt/ml/input/data/all_hin_articles.json")
s3.download_file(s3_bucket, "data/train.bin", "/opt/ml/input/data/train.bin")
...
class RAGGPT(nn.Module):
    ...
    def forward(self, idx, targets=None):
        ...
        with torch.no_grad():
            ...
            ctx_emb = torch.stack([
                self.retriever.embed_model.encode(c)[:32]  # First 32 tokens
                for c in ctx
                ]).to(device)
        ...
        x = torch.cat([ctx_emb, x.unsqueeze(1).expand(-1,3,-1,-1)], dim=2)
...
def main():
    # Data
    tokenizer = ByteLevelBPETokenizer(f"{local_tokenizer_path}/vocab.json",
    f"{local_tokenizer_path}/merges.txt")
    encode = lambda s: tokenizer.encode(s).ids
    decode = lambda l: tokenizer.decode(l)
    
    # Retriever
    embed_model = SentenceTransformer('sentence-transformers/paraphrase-multilingual-mpnet-base-v2').to(device)
    with open("/opt/ml/input/data/all_hin_articles.json", "r", encoding="utf-8") as f:
    ...
    train_loader = torch.utils.data.DataLoader(
        BinaryDataset("/opt/ml/input/data/train.bin"),
