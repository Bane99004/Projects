...
class FAISSRetriever(nn.Module):
    def __init__(self, index_path="faiss_index"):
        ...
        self.embed_model = SentenceTransformer(modules=[word_embedding_model, Pooling(word_embedding_model.get_word_embedding_dimension())]).to(self.device)

    def retrieve(self, queries, k=3):
        query_embs = self.embed_model.encode(queries, device=self.device, convert_to_numpy=True, normalize_embeddings=True)
...
class RAGGPT(nn.Module):
    ...
    def forward(self, idx):
        ...
        ctx_emb = torch.stack([self.retriever.embed_model.encode(c, convert_to_tensor=True).to(device) for c in ctx])
...
@app.get("/gptinterface", response_class=HTMLResponse)
async def gpt_interface(request: Request):
    return templates.TemplateResponse("gpt_interface.html", {"request": request})

# Text generation API
@app.post("/gptinterface/generate")
async def generate_text(data: PromptInput):
    response = model.generate(data.prompt, max_new_tokens=data.max_length)
    return {"response": response}

if __name__ == "__main__":
    uvicorn.run(app, host="13.201.77.21", port=8000)
